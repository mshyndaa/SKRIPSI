    ...
    <body style="vertical-align: middle;text-align: center;">
                        ...
                        <tr>
                            <td>
                                ...
                                <form action="/save" method="post">
                                    ...
                                    <br id="brname" style="display:none">
                                    <label for="idn" id='idtext' hidden>Source ID</label>
                                    <input type="hidden" id="idn" name="idn" value="">
                                    <br id='brtext'>
                                    <label for="link" id="linktext" >Link</label>
                                    <input type="text" id="link" style="width:70%;" name="link" value="">
                                    <br id='brlink'>
                                    <label for="link" id="idlinktext" >ID</label>
                                    <input type="text" id="idlink" name="idlink" value="" maxlength="5">
                                    <br id='bridlink'>
        ...
        <script>
                            ...
                            function changemaps(a) {
                                ...
                                $.ajax({
                                    ...
                                    success: function (response) {
                                        ...
                                        $.ajax({
                                            type: 'GET',
                                            url: window.location.origin + "/flooradmin/" + idfloor,
                                            beforeSend: function () {
                                                div.innerHTML = '';
                                            },
                                            success: function (response) {
                                                var data = JSON.parse(response)
                                                if (data.length != 0) {
                                                     div.innerHTML += '<rect  style="background-color: blue;fill-opacity:0;"  x="0" y="0" fill="blue" width="100%" height="100%"/>';
                                                    for (var i = 0; i < data.length; i++) {
                                                        if (data[i]['type'] == 'hibob') {
                                                            var x = data[i]['x_axis'];
                                                            var y = data[i]['y_axis'];
                                                            var id = data[i]['id'];
                                                            div.innerHTML += '<circle id="c3_' + id + '" cx="' + x + '" cy="' + y + '" r="8" fill="#21de54" onclick="deleteddata(' + id + ')"></circle>';
                            ...
                            function makeDrag(id) {
                                //draggable = new PlainDraggable(document.getElementById('c322'));
                                ...
                                draggable.onDragEnd = function (newPosition) {
                                    ...
                                    //The cursor point, translated into svg coordinates
                                    var cursorpt = pt.matrixTransform(svg.getScreenCTM().inverse());
                                    console.log("(aaaa" + cursorpt.x + ", " + cursorpt.y + ")");
                                    if (cursorpt.x == x_val && cursorpt.y == y_val) {
                                        document.getElementById("mydiv").removeAttribute("hidden");
                                        document.getElementById("deleteid").value = id;
                                    }
                                };
                                draggable.onDragStart = function (newPosition) {
                                    console.log(newPosition.top);
                                    pt.x = newPosition.left;
                                    pt.y = newPosition.top;
                                    console.log("baru" + pt.x + ", " + pt.y + ")");
                            ...
                            function inmouse(a, index1) {
                                ...
                                document.getElementById("text" + index1).style.visibility = "visi ble"
